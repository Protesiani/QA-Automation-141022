    ...
    public void serveCardsToPlayer(int noOfPlayers, Deck deck) throws NotEnoughPlayerException {
        ...
        for (int round = 1; round <= 3; round++) {
            ...
            for (int player = 1; player <= noOfPlayers; player++) {
                ...
                System.out.print(String.format("Player %d gets card %s, ", player, deck.takeCard(index)));
                index++;
            }
            System.out.println();
        }
        System.out.println(String.format("%n%d cards left in deck", deck.currentSize()));
        hands[2].viewHand();
        System.out.println("");
        hands[2].findCards("Queen", "Spades", hands[2].viewHand());
    }
}